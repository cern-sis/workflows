...
def get_latest_s3_file(s3_bucket, aws_conn_id):
...
def jagiellonian_pull_api():
    ...
    def fetch_crossref_api():
        ...
        endpoint_filter = ""
        if get_latest_s3_file(s3_bucket, aws_conn_id):
            endpoint_filter = f"&filter=from-created-date:{get_latest_s3_file(s3_bucket, aws_conn_id)}"
        ...
        while total_results is None or current_offset < total_results:
            endpoint = f"journals/{jagiellonian_issn}/works?rows={rows_per_page}&offset={current_offset}{endpoint_filter}"
